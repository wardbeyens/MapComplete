...
export class AllKnownLayouts {
  public static allKnownLayouts: Map<string, LayoutConfig> =
    AllKnownLayouts.AllLayouts();
  public static layoutsList: LayoutConfig[] =
    AllKnownLayouts.GenerateOrderedList(AllKnownLayouts.allKnownLayouts);

  private static GenerateOrderedList(
    allKnownLayouts: Map<string, LayoutConfig>
  ): LayoutConfig[] {
    const keys = [
      "personal",
      "cyclofix",
  ...
  private static AddGhostBikes(layout: LayoutConfig): LayoutConfig {
    ...
    if (date === "31/10" || date === "1/11" || date === "2/11") {
      console.log(
        "The current date is ",
        date,
        ", which means we remember our dead"
      );
      // Around Halloween/Fiesta de muerte/Allerzielen, we remember the dead
      layout.layers.push(AllKnownLayers.sharedLayers.get("ghost_bike"));
    }
    return layout;
  }
      ...
      for (let i = 0; i < layout.layers.length; i++) {
        ...
        if (typeof layer === "string") {
          ...
          if (layer === undefined) {
            console.log(
              "Defined layers are ",
              AllKnownLayers.sharedLayers.keys()
            );
