...
export class FromJSON {
  public static SimpleTag(json: string, context?: string): Tag {
    ...
    if (tag.length !== 2) {
      throw `Invalid tag: no (or too much) '=' found (in ${
        context ?? "unkown context"
      })`;
    }
  ...
  public static Tag(
    json: AndOrTagConfigJson | string,
    context: string = ""
  ): TagsFilter {
    try {
  ...
  private static TagUnsafe(
    ...
    if (typeof json == "string") {
      ...
      if (tag.indexOf("!~") >= 0) {
        ...
        return new RegexTag(split[0], new RegExp("^" + split[1] + "$"), true);
      }
      if (tag.indexOf("~~") >= 0) {
        const split = Utils.SplitFirst(tag, "~~");
        if (split[1] === "*") {
      ...
      if (tag.indexOf("~") >= 0) {
        ...
        return new RegexTag(split[0], new RegExp("^" + split[1] + "$"));
      }
      if (tag.indexOf("=") >= 0) {
        const split = Utils.SplitFirst(tag, "=");
        if (split[1] == "*") {
