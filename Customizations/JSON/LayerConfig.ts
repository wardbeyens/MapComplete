...
export default class LayerConfig {
  ...
  constructor(json: LayerConfigJson, units?: Unit[], context?: string, official: boolean = true) {
    this.units = units ?? [];
    ...
    const mappedHtml = tags.map((tgs) => {
      function genHtmlFromString(sourcePart: string): BaseUIElement {
        ...
        if (match !== null && Svg.All[match[1] + '.svg'] !== undefined) {
          html = new Combine([(Svg.All[match[1] + '.svg'] as string).replace(/#000000/g, match[2])]).SetStyle(style);
        }
        return html;
      }
      ...
      for (const iconOverlay of self.iconOverlays) {
        ...
        if (iconOverlay.badge) {
          ...
          const badgeCompound = new Combine(badgeParts).SetStyle('display:flex;position:relative;width:100%;height:100%;');

          badges.push(badgeCompound);
        } else {
          htmlParts.push(genHtmlFromString(iconOverlay.then.GetRenderValue(tgs).txt));
        }
      }

      if (badges.length > 0) {
        const badgesComponent = new Combine(badges).SetStyle('display:flex;height:50%;width:100%;position:absolute;top:50%;left:50%;');
