  ...
  constructor(json: LayoutConfigJson, official = true, context?: string) {
    ...
    this.shortDescription =
      json.shortDescription === undefined
        ...
        : new Translation(json.shortDescription, context + ".shortdescription");
    this.descriptionTail =
      json.descriptionTail === undefined
        ? new Translation({ "*": "" }, context + ".descriptionTail")
        : new Translation(json.descriptionTail, context + ".descriptionTail");
    this.icon = json.icon;
    ...
    if (json.clustering) {
      ...
      for (const layer of this.layers) {
        if (layer.wayHandling !== LayerConfig.WAYHANDLING_CENTER_ONLY) {
          console.debug(
            "WARNING: In order to allow clustering, every layer must be set to CENTER_ONLY. Layer",
            layer.id,
            "does not respect this for layout",
            this.id
          );
        }
      }
    ...
    this.enableBackgroundLayerSelection =
      json.enableBackgroundLayerSelection ?? true;
  ...
  private static ExtractLayers(
    ...
    json.layers.forEach((layer, i) => {
      if (typeof layer === "string") {
        if (AllKnownLayers.sharedLayersJson[layer] !== undefined) {
          if (json.overrideAll !== undefined) {
            let lyr = JSON.parse(
              JSON.stringify(AllKnownLayers.sharedLayersJson[layer])
            );
            const newLayer = new LayerConfig(
              Utils.Merge(json.overrideAll, lyr),
              units,
              `${json.id}+overrideAll.layers[${i}]`,
              official
            );
      ...
      names.forEach((name) => {
        ...
        // @ts-ignore
        let newLayer: LayerConfigJson = Utils.Merge(
          layer.override,
          JSON.parse(JSON.stringify(shared))
        ); // We make a deep copy of the shared layer, in order to protect it from changes
        if (json.overrideAll !== undefined) {
  ...
  private static ExtractUnits(json: LayoutConfigJson, context: string): Unit[] {
    ...
    if ((json.units ?? []).length !== 0) {
      for (let i1 = 0; i1 < json.units.length; i1++) {
        ...
        const defaultSet = unit.applicableUnits.filter(
          (u) => u.default === true
        );
        ...
        if (defaultSet.length > 1) {
          throw `Multiple units are set as default: they have canonical values of ${defaultSet
            .map((u) => u.canonicalDenomination)
            .join(", ")}`;
        }
        const applicable = unit.applicableUnits.map(
          (u, i) => new Denomination(u, `${context}.units[${i}]`)
        );
        result.push(
          new Unit(appliesTo, applicable, unit.eraseInvalidValues ?? false)
        );
      }

      const seenKeys = new Set<string>();
      for (const unit of result) {
        const alreadySeen = Array.from(unit.appliesToKeys).filter(
          (key: string) => seenKeys.has(key)
        );
        if (alreadySeen.length > 0) {
          throw `${context}.units: multiple units define the same keys. The key(s) ${alreadySeen.join(
            ","
