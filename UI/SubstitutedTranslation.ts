...
export class SubstitutedTranslation extends VariableUiElement {
  public constructor(translation: Translation, tagsSource: UIEventSource<any>, mapping: Map<string, BaseUIElement> = undefined) {
    ...
    super(
      Locale.language.map((language) => {
        ...
        return new Combine(
          SubstitutedTranslation.ExtractSpecialComponents(txt, extraMappings).map((proto) => {
            ...
            try {
              ...
              return new FixedUiElement(`Could not generate special rendering for ${viz.func}(${viz.args.join(', ')}) ${e}`).SetStyle(
                'alert'
              );
            }
          })
  ...
  public static ExtractSpecialComponents(
    ...
    if (template.search(/{[^}]+\([^}]*\)}/) >= 0) {
      ...
      console.warn(
        ...
        template,
        ' did you mean one of: ',
        SpecialVisualizations.specialVisualizations.map((sp) => sp.funcName + '()').join(', ')
      );
    }
