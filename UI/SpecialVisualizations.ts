...
export default class SpecialVisualizations {
  ...
  static constructShowDataLayer: (
    features: UIEventSource<{ feature: any; freshness: Date }[]>,
    leafletMap: UIEventSource<any>,
    layoutToUse: UIEventSource<any>,
  ...
  public static specialVisualizations: SpecialVisualization[] = [
    ...
    },
    {
      ...
      constr: (state, tagSource, args) => {
        ...
        SpecialVisualizations.constructShowDataLayer(featuresToShow, minimap['leafletMap'], State.state.layoutToUse, false, true);
...

    {
      ...
      constr: (state: State, tagSource: UIEventSource<any>, args: string[]) => {
        ...
        const listSource: UIEventSource<string[]> = tagSource.map((tags) => {
          try {
  ...
  private static GenHelpMessage() {
    const helpTexts = SpecialVisualizations.specialVisualizations.map(
      (viz) =>
        new Combine([
          ...
          new FixedUiElement(viz.example ?? '{' + viz.funcName + '(' + viz.args.map((arg) => arg.defaultValue).join(',') + ')}').SetClass(
            'literal-code'
          )
        ])
    );
