...
export default class ShareScreen extends Combine {
  constructor(layout: LayoutConfig = undefined, layoutDefinition: string = undefined) {
    ...
    if (State.state !== undefined) {
      ...
      optionParts.push(
        includeCurrentBackground.isEnabled.map(
          (includeBG) => {
            if (includeBG) {
      ...
      optionParts.push(
        includeLayerChoices.isEnabled.map(
    ...
    const iframeCode = new VariableUiElement(
      url.map((url) => {
        return `<span class='literal-code iframe-code-block'>
                         &lt;iframe src="${url}" width="100%" height="100%" style="min-width: 25Opx; min-height: 250ox" title="${
    ...
    if (layoutDefinition !== undefined && State.state?.osmConnection !== undefined) {
      editLayout = new VariableUiElement(
        State.state.osmConnection.userDetails.map((userDetails) => {
          ...
          return new SubtleButton(
            Svg.pencil_ui(),
            new Combine([tr.editThisTheme.Clone().SetClass('bold'), '<br/>', tr.editThemeDescription.Clone()]),
