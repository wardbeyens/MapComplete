...
export default class SimpleAddUI extends Toggle {
  constructor(isShown: UIEventSource<boolean>) {
    const loginButton = new SubtleButton(Svg.osm_logo_ui(), Translations.t.general.add.pleaseLogin.Clone()).onClick(() =>
    ...
    const addUi = new VariableUiElement(
      selectedPreset.map((preset) => {
        ...
        return SimpleAddUI.CreateConfirmButton(
          preset,
          (tags) => {
            createNewPoint(tags);
    ...
    super(
      new Toggle(
        new Toggle(
          new Toggle(Translations.t.general.add.stillLoading.Clone().SetClass('alert'), addUi, State.state.layerUpdater.runningQuery),
        ...
        State.state.osmConnection.userDetails.map(
          (userdetails: UserDetails) =>
            userdetails.csCount >= Constants.userJourney.addNewPointWithUnreadMessagesUnlock || userdetails.unreadMessages == 0
        )
      ),
      loginButton,
  ...
  private static CreateConfirmButton(preset: PresetInfo, confirm: (tags: any[]) => void, cancel: () => void): BaseUIElement {
    ...
    const openLayerControl = new SubtleButton(
      Svg.layers_ui(),
      new Combine([
        Translations.t.general.add.layerNotEnabled.Subs({ layer: preset.layerToAddTo.layerDef.name }).SetClass('alert'),
        Translations.t.general.add.openLayerControl
      ])
    ).onClick(() => State.state.layerControlIsOpened.setData(true));

    const openLayerOrConfirm = new Toggle(confirmButton, openLayerControl, preset.layerToAddTo.isDisplayed);
    const tagInfo = SimpleAddUI.CreateTagInfoFor(preset);

    const cancelButton = new SubtleButton(Svg.close_ui(), Translations.t.general.cancel).onClick(cancel);

    return new Combine([
      Translations.t.general.add.confirmIntro.Subs({ title: preset.name }),
      State.state.osmConnection.userDetails.data.dryRun ? Translations.t.general.testing.Clone().SetClass('alert') : undefined,
      openLayerOrConfirm,
      cancelButton,
      preset.description,
      tagInfo
  ...
  private static CreateTagInfoFor(preset: PresetInfo, optionallyLinkToWiki = true) {
    ...
    return new Toggle(
      Translations.t.general.add.presetInfo
        .Subs({
          tags: preset.tags
            .map((t) => t.asHumanString(optionallyLinkToWiki && csCount > Constants.userJourney.tagsVisibleAndWikiLinked, true))
            .join('&')
        })
  ...
  private static CreatePresetSelectButton(preset: PresetInfo) {
    ...
    return new SubtleButton(
      preset.icon,
      new Combine([
        Translations.t.general.add.addNew
          .Subs({
  ...
  private static CreatePresetButtons(selectedPreset: UIEventSource<PresetInfo>): BaseUIElement {
    ...
    for (const layer of State.state.filteredLayers.data) {
      ...
      for (const preset of presets) {
        ...
        let icon: BaseUIElement = layer.layerDef
          .GenerateLeafletStyle(new UIEventSource<any>(tags), false)
          .icon.html.SetClass('w-12 h-12 block relative');
