...
export default class MoreScreen extends Combine {
  ...
  private static Init(onMainScreen: boolean, state: State): BaseUIElement[] {
    ...
    if (onMainScreen) {
      intro = new Combine([
        LanguagePicker.CreateLanguagePicker(Translations.t.index.title.SupportedLanguages()).SetClass('absolute top-2 right-3'),
        new IndexText()
      ]);
  ...
  private static createUnofficialThemeList(buttonClass: string): BaseUIElement {
    return new VariableUiElement(
      State.state.installedThemes.map((customThemes) => {
