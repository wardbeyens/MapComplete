...
export default class LayerSelection extends Combine {
  constructor(
    ...
    for (const layer of activeLayers.data) {
      const leafletStyle = layer.layerDef.GenerateLeafletStyle(new UIEventSource<any>({ id: 'node/-1' }), false);
      const leafletStyleNa = layer.layerDef.GenerateLeafletStyle(new UIEventSource<any>({ id: 'node/-1' }), false);
      ...
      const zoomStatus = new VariableUiElement(
      ...
      const zoomStatusNonActive = new VariableUiElement(
        State.state.locationControl.map((location) => {
          if (location.zoom < layer.layerDef.minzoom) {
            return Translations.t.general.layerSelection.zoomInToSeeThisLayer
              .Clone()
      ...
      checkboxes.push(
        new Toggle(
          new Combine([new Combine([icon, name.Clone()]).SetStyle(style), zoomStatus]).SetStyle(styleWhole),
          new Combine([new Combine([iconUnselected, '<del>', name.Clone(), '</del>']).SetStyle(style), zoomStatusNonActive]).SetStyle(
            styleWhole
          ),
          layer.isDisplayed
        )
