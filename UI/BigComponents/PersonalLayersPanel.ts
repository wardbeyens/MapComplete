...
export default class PersonalLayersPanel extends VariableUiElement {
  constructor() {
    super(
      State.state.installedThemes.map((installedThemes) => {
        ...
        const allThemes = AllKnownLayouts.layoutsList
          .concat(installedThemes.map((layout) => layout.layout))
        ...
        return new Toggle(
          new Combine([t.panelIntro.Clone(), panel]).SetClass('flex flex-col'),
          new SubtleButton(Svg.osm_logo_ui(), t.loginNeeded.Clone().SetClass('text-center')).onClick(() =>
  ...
  private static CreateLayerToggle(layer: LayerConfig): Toggle {
    let icon: BaseUIElement = new Combine([
      layer.GenerateLeafletStyle(new UIEventSource<any>({ id: 'node/-1' }), false, '2em').icon.html
    ]).SetClass('relative');
    let iconUnset = new Combine([layer.GenerateLeafletStyle(new UIEventSource<any>({ id: 'node/-1' }), false, '2em').icon.html]).SetClass(
      'relative'
    );
    ...
    return new Toggle(
      new SubtleButton(icon, content),
      new SubtleButton(iconUnset, contentUnselected),
      State.state.favouriteLayers.map(
        (favLayers) => {
