...
export default class Attribution extends Combine {
  constructor(
    ...
    const idLink = location.map(
      (location) => `https://www.openstreetmap.org/edit?editor=id#map=${location?.zoom ?? 0}/${location?.lat ?? 0}/${location?.lon ?? 0}`
    );
    const editHere = new Link(Svg.pencil_ui().SetClass('small-image'), idLink, true);

    const mapillaryLink = location.map(
      (location) =>
        `https://www.mapillary.com/app/?focus=map&lat=${location?.lat ?? 0}&lng=${location?.lon ?? 0}&z=${Math.max(
          (location?.zoom ?? 2) - 1,
          1
        )}`
    );
