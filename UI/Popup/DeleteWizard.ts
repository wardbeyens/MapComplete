...
export default class DeleteWizard extends Toggle {
   ...
   */
  constructor(id: string, options: DeleteConfig) {
    ...
    function softDelete(reason: string, tagsToApply: { k: string; v: string }[]) {
      ...
      (State.state?.changes ?? new Changes()).addTag(id, new And(tagsToApply.map((kv) => new Tag(kv.k, kv.v))), tagsSource);
    }
    ...
    function doDelete(selected: TagsFilter) {
      ...
      if (deleteReasonMatch.length > 0) {
        ...
        deleteAction.DoDelete(deleteReason, () => {
          ...
          softDelete(
            deleteReason,
            tgs.filter((kv) => kv.k !== '_delete_reason')
          );
        });
        return;
      } else {
    ...
    const question = new VariableUiElement(
      tagsSource.map((currentTags) => {
        const config = DeleteWizard.generateDeleteTagRenderingConfig(
          options.softDeletionTags,
          options.nonDeleteMappings,
          options.extraDeleteReasons,
          currentTags
        );
        return new TagRenderingQuestion(tagsSource, config, {
          cancelButton: cancelButton,
          /*Using a custom save button constructor erases all logic to actually save, so we have to listen for the click!*/
          saveButtonConstr: (v) =>
            DeleteWizard.constructConfirmButton(v).onClick(() => {
              doDelete(v.data);
            }),
     ...
     */
    const deleteButton = new SubtleButton(Svg.delete_icon_ui().SetStyle('width: 2rem; height: 2rem;'), t.delete.Clone())
      .onClick(() => {
    ...
    super(
      new Toggle(
        new Combine([Svg.delete_icon_svg().SetClass('h-16 w-16 p-2 m-2 block bg-gray-300 rounded-full'), t.isDeleted.Clone()]).SetClass(
          'flex m-2 rounded-full'
        ),
        new Toggle(
          new Toggle(
            new Toggle(
              new Toggle(
                ...
                State.state.osmConnection.userDetails.map(
                  (ud) => ud.csCount > Constants.userJourney.addNewPointWithUnreadMessagesUnlock || ud.unreadMessages == 0
                )
              ),
  ...
  private static constructConfirmButton(deleteReasons: UIEventSource<TagsFilter>): BaseUIElement {
    const t = Translations.t.delete;
    const btn = new Combine([Svg.delete_icon_ui().SetClass('w-6 h-6 mr-3 block'), t.delete.Clone()]).SetClass('flex btn bg-red-500');

    const btnNonActive = new Combine([Svg.delete_icon_ui().SetClass('w-6 h-6 mr-3 block'), t.delete.Clone()]).SetClass(
      'flex btn btn-disabled bg-red-200'
    );
    ...
    return new VariableUiElement(
      tags.map(
        (currentTags) => {
          ...
          return new Combine([
            t.explanations.softDelete.Subs({ reason: cbd.reason }),
  ...
  private static generateDeleteTagRenderingConfig(
    ...
    return new TagRenderingConfig(
      {
        ...
        mappings: [
          ...
          {
            if: {
              and: ['_delete_reason=testing point', ...softDeletionTagsStr]
            },
            then: t.reasons.test
          },
          {
            if: {
              and: ['_delete_reason=disused', ...softDeletionTagsStr]
            },
            then: t.reasons.disused
          },
          {
            if: {
              and: ['_delete_reason=not found', ...softDeletionTagsStr]
            },
