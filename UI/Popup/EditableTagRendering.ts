...
export default class EditableTagRendering extends Toggle {
  constructor(tags: UIEventSource<any>, configuration: TagRenderingConfig, units: Unit[], editMode = new UIEventSource<boolean>(false)) {
    ...
    if (configuration.question !== undefined && State.state?.featureSwitchUserbadge?.data) {
      ...
      const editButton = new Combine([Svg.pencil_ui()])
      ...
      const answerWithEditButton = new Combine([answer, new Toggle(editButton, undefined, State.state.osmConnection.isLoggedIn)]).SetClass(
    ...
    const renderingIsShown = tags.map((tags) => configuration.IsKnown(tags) && (configuration?.condition?.matchesProperties(tags) ?? true));
    super(rendering, undefined, renderingIsShown);
  }
}
