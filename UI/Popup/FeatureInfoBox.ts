...
export default class FeatureInfoBox extends ScrollableFullScreen {
  public constructor(tags: UIEventSource<any>, layerConfig: LayerConfig) {
    super(
  ...
  private static GenerateTitleBar(tags: UIEventSource<any>, layerConfig: LayerConfig): BaseUIElement {
    const title = new TagRenderingAnswer(tags, layerConfig.title ?? new TagRenderingConfig('POI', undefined)).SetClass(
      'break-words font-bold sm:p-0.5 md:p-1 sm:p-1.5 md:p-2'
    );
    const titleIcons = new Combine(
      layerConfig.titleIcons.map((icon) => new TagRenderingAnswer(tags, icon, 'block w-8 h-8 align-baseline box-content sm:p-0.5'))
    ).SetClass('flex flex-row flex-wrap pt-0.5 sm:pt-1 items-center mr-2');

    return new Combine([new Combine([title, titleIcons]).SetClass('flex flex-col sm:flex-row flex-grow justify-between')]);
  }

  private static GenerateContent(tags: UIEventSource<any>, layerConfig: LayerConfig): BaseUIElement {
    ...
    if (layerConfig.deletion) {
      renderings.push(new VariableUiElement(tags.map((tags) => tags.id).map((id) => new DeleteWizard(id, layerConfig.deletion))));
    }
