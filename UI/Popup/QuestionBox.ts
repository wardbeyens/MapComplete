...
export default class QuestionBox extends VariableUiElement {
  constructor(tagsSource: UIEventSource<any>, tagRenderings: TagRenderingConfig[], units: Unit[]) {
    ...
    tagRenderings = tagRenderings.filter((tr) => tr.question !== undefined).filter((tr) => tr.question !== null);
      ...
      tagsSource.map(
        (tags) => {
          ...
          const tagRenderingQuestions = tagRenderings.map(
            (tagRendering, i) =>
              new TagRenderingQuestion(tagsSource, tagRendering, {
                units: units,
                afterSave: () => {
                  // We save
          ...
          const skippedQuestionsButton = Translations.t.general.skippedQuestions.Clone().onClick(() => {
            skippedQuestions.setData([]);
          });

          const allQuestions: BaseUIElement[] = [];
          for (let i = 0; i < tagRenderingQuestions.length; i++) {
            const tagRendering = tagRenderings[i];
