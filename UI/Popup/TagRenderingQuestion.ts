...
export default class TagRenderingQuestion extends Combine {
  constructor(
    ...
    const question = new SubstitutedTranslation(configuration.question, tags).SetClass('question-text');
    ...
    const save = () => {
      ...
      if (selection) {
        (State.state?.changes ?? new Changes()).addTag(tags.data.id, selection, tags);
      }
    ...
    if (options.saveButtonConstr === undefined) {
      options.saveButtonConstr = (v) => new SaveButton(v, State.state?.osmConnection).onClick(save);
    }
    ...
    if (options.bottomText !== undefined) {
      ...
      bottomTags = new VariableUiElement(
        inputElement.GetValue().map((tagsFilter: TagsFilter) => {
    ...
    super([question, inputElement, options.cancelButton, saveButton, bottomTags]);
  ...
  private static GenerateInputElement(
    ...
    const mappings = (configuration.mappings ?? []).filter((mapping) => {
    ...
    if (configuration.multiAnswer) {
      return TagRenderingQuestion.GenerateMultiAnswer(
        ...
        inputEls,
        ff,
        configuration.mappings.map((mp) => mp.ifnot)
      );
    } else {
  ...
  private static GenerateMultiAnswer(
    configuration: TagRenderingConfig,
    elements: InputElement<TagsFilter>[],
    freeformField: InputElement<TagsFilter>,
    ifNotSelected: TagsFilter[]
  ): InputElement<TagsFilter> {
  ...
  private static GenerateMappingElement(
    ...
    return new FixedInputElement(new SubstitutedTranslation(mapping.then, tagsSource), tagging, (t0, t1) => t1.isEquivalent(t0));
  }
  ...
  private static GenerateFreeform(
    ...
    let inputTagsFilter: InputElement<TagsFilter> = new InputElementMap(
      input,
      (a, b) => a === b || (a?.isEquivalent(b) ?? false),
      pickString,
      toString
    );
