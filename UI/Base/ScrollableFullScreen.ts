...
export default class ScrollableFullScreen extends UIElement {
  ...
  constructor(
    title: (mode: string) => BaseUIElement,
    ...
    this._component = this.BuildComponent(title('desktop'), content('desktop'), isShown).SetClass('hidden md:block');
  ...
  private BuildComponent(title: BaseUIElement, content: BaseUIElement, isShown: UIEventSource<boolean>) {
    const returnToTheMap = new Combine([Svg.back_svg().SetClass('block md:hidden'), Svg.close_svg().SetClass('hidden md:block')])
      .onClick(() => {
    ...
    const ornament = new Combine([new Ornament().SetStyle('height:5em;')]).SetClass('md:hidden h-5');
    return new Combine([
      new Combine([
        new Combine([returnToTheMap, title]).SetClass(
          'border-b-2 border-black shadow md:shadow-none bg-white p-2 pb-0 md:p-0 flex flex-shrink-0'
        ),
        new Combine([content, ornament]).SetClass('block p-2 md:pt-4 w-full h-full overflow-y-auto md:max-h-65vh')
