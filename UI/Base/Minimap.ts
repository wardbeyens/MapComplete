...
export default class Minimap extends BaseUIElement {
  ...
  constructor(options?: { background?: UIEventSource<BaseLayer>; location?: UIEventSource<Loc>; allowMoving?: boolean }) {
    super();
    options = options ?? {};
  ...
  private InitMap() {
    ...
    map.on('moveend', function () {
      ...
      if (map.getZoom() === location.data.zoom && map.getCenter().lat === location.data.lat && map.getCenter().lng === location.data.lon) {
        return;
      }
