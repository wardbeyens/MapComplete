...
export class SubtleButton extends UIElement {
  ...
  constructor(
    ...
    linkTo: { url: string | UIEventSource<string>; newTab?: boolean } = undefined
  ) {
    super();
  ...
  private static generateContent(
    ...
    const image = new Combine([img]).SetClass('flex-shrink-0');

    if (linkTo == undefined) {
      return new Combine([image, message?.SetClass('block overflow-ellipsis')]).SetClass('flex group w-full');
    }

    return new Link(
      new Combine([image, message?.SetClass('block overflow-ellipsis')]).SetClass('flex group w-full'),
      linkTo.url,
      linkTo.newTab ?? false
    );
  }
