...
export default class DirectionInput extends InputElement<string> {
  ...
  constructor(mapBackground: UIEventSource<any>, location: UIEventSource<Loc>, value?: UIEventSource<string>) {
    super();
  ...
  protected InnerConstructElement(): HTMLElement {
    ...
    const element = new Combine([
      Svg.direction_stroke_svg()
        .SetStyle(`position: absolute;top: 0;left: 0;width: 100%;height: 100%;transform:rotate(${this.value.data ?? 0}deg);`)
        .SetClass('direction-svg relative')
        .SetStyle('z-index: 1000'),
      map.SetClass('w-full h-full absolute top-0 left-O rounded-full overflow-hidden')
    ])
      .SetStyle(
        'position:relative;display:block;width: min(100%, 25em); height: min(100% , 25em); background:white; border: 1px solid black; border-radius: 999em'
      )
      .ConstructElement();
