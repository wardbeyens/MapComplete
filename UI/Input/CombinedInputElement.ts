...
export default class CombinedInputElement<T, J, X> extends InputElement<X> {
  ...
  constructor(a: InputElement<T>, b: InputElement<J>, combine: (t: T, j: J) => X, split: (x: X) => [T, J]) {
    super();
    this._a = a;
    this._b = b;
    this._split = split;
    this.IsSelected = this._a.IsSelected.map(
      (isSelected) => {
    ...
    this._value = this._a
      .GetValue()
      .map((t) => combine(t, this._b?.GetValue()?.data), [this._b.GetValue()])
      .addCallback((x) => {
        const [t, j] = split(x);
