...
export default class OpeningHoursVisualization extends Toggle {
  ...
  private static ConstructVizTable(
    oh: any,
    ranges: { isOpen: boolean; isSpecial: boolean; comment: string; startDate: Date; endDate: Date }[][],
    rangeStart: Date
  ): BaseUIElement {
  ...
  private static CreateRangeElem(
    ...
    return new FixedUiElement(textToShow).SetStyle(`left:${startPercentage}%; width:${width}%`).SetClass('ohviz-range');
  }
