  ...
  constructor(subject: string, reviews: UIEventSource<Review[]>, middleElement: BaseUIElement) {
    super(
      reviews.map((revs) => {
          ...
          new Combine([
            ...
            new Link(
              revs.length === 1
                ...
                : Translations.t.reviews.title.Clone().Subs({ count: '' + revs.length }),
        ...
        elements.push(
          new Combine([Translations.t.reviews.attribution.Clone(), new Img('./assets/mangrove_logo.png')]).SetClass('review-attribution')
        );
