    ...
    const uiElements = images.map((imageURLS: { key: string; url: string }[]) => {
      ...
      for (const url of imageURLS) {
        ...
        if (url.key !== undefined) {
          image = new Combine([image, new DeleteImage(url.key, tags).SetClass('delete-image-marker absolute top-0 left-0 pl-3')]).SetClass(
            'relative'
          );
        }
        image.SetClass('w-full block').SetStyle('min-width: 50px; background: grey;');
