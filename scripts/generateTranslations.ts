...
function compileTranslationsFromWeblate() {
  const translations = ScriptUtils.readDirRecSync('./langs', 1).filter((path) => path.indexOf('.json') > 0);
...
function loadTranslationFilesFrom(target: string): Map<string, any> {
  const translationFilePaths = ScriptUtils.readDirRecSync('./langs/' + target).filter((path) => path.endsWith('.json'));
