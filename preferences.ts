...
function createTable(preferences: any) {
  ...
  for (const key in preferences) {
    ...
    if (connection.userDetails.data.csCount > 500 && (key.startsWith('mapcomplete') || connection.userDetails.data.csCount > 2500)) {
      value = new TextField({
        value: pref
      });
    }

    const row = [key, new Button('delete', () => pref.setData(null)), value];
    prefs.push(row);
  }

  new Combine([
    ...salvageThemes(preferences).map((theme) => SalvageButton(theme)),
    new Table(['Key', '', 'Value'], prefs),
    new SubtleButton('./assets/svg/delete_icon.svg', 'Delete all mapcomplete preferences (mangrove identies are preserved)').onClick(() =>
