...
export default class SimpleMetaTagger {
  ...
  public static readonly objectMetaInfo = new SimpleMetaTagger(
    {
      keys: ['_last_edit:contributor', '_last_edit:contributor:uid', '_last_edit:changeset', '_last_edit:timestamp', '_version_number'],
      doc: 'Information about the last edit of this object.'
    },
    (feature) => {
      /*Note: also handled by 'UpdateTagsFromOsmAPI'*/
      ...
      tagsSource.addCallbackAndRunD((tags) => {
        ...
        try {
          ...
          const updateTags = () => {
            ...
            if (oldNextChange > new Date().getTime() && tags['_isOpen:oldvalue'] === tags['opening_hours']) {
  ...
  private static carriageWayWidth = new SimpleMetaTagger(
    ...
    (feature) => {
      ...
      const _leftSideParking = new And([new Tag('parking:lane:left', 'parallel'), new Tag('parking:lane:right', 'no_parking')]);
      const _rightSideParking = new And([new Tag('parking:lane:right', 'parallel'), new Tag('parking:lane:left', 'no_parking')]);

      const _bothSideParking = new Tag('parking:lane:both', 'parallel');
      const _noSideParking = new Tag('parking:lane:both', 'no_parking');
      const _otherParkingMode = new Or([
      ...
      let onewayBike = properties['oneway:bicycle'] === 'yes' || (onewayCar && properties['oneway:bicycle'] === undefined);

      let cyclingAllowed = !(properties.bicycle === 'use_sidepath' || properties.bicycle === 'no');
      ...
      const targetWidthIgnoringPedestrians = carWidthUsed + cyclistWidthUsed + parallelParkingCount * carWidthUsed;
  ...
  static HelpText(): BaseUIElement {
    ...
    for (const metatag of SimpleMetaTagger.metatags) {
      subElements.push(new Title(metatag.keys.join(', '), 3), metatag.doc);
    }
