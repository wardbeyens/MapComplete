...
export default class MangroveReviews {
  ...
  private constructor(lon: number, lat: number, name: string, identity: MangroveIdentity, dryRun?: boolean) {
    this._lon = lon;
    this._lat = lat;
    this._name = name;
  ...
  public static Get(lon: number, lat: number, name: string, identity: MangroveIdentity, dryRun?: boolean) {
  ...
  public GetReviews(): UIEventSource<Review[]> {
    if (this._lastUpdate !== undefined && this._reviews.data !== undefined && new Date().getTime() - this._lastUpdate.getTime() < 15000) {
