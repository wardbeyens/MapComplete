...
export class ChangesetHandler {
    ...
    if (this.currentChangeset.data === undefined || this.currentChangeset.data === '') {
      ...
      this.OpenChangeset(
        ...
        (csId) => {
          ...
          self.AddChange(csId, changeset, allElements, whenDone, (e) => {
      ...
      self.AddChange(csId, generateChangeXML(csId), allElements, whenDone, (e) => {
   ...
   *
   */
  public DeleteElement(object: OsmObject, layout: LayoutConfig, reason: string, allElements: ElementStorage, continuation: () => void) {
    function generateChangeXML(csId: string) {
      let [lat, lon] = object.centerpoint();

      let changes = `<osmChange version='0.6' generator='Mapcomplete ${Constants.vNumber}'>`;
      changes += `<delete><${object.type} id="${object.id}" version="${object.version}" changeset="${csId}" lat="${lat}" lon="${lon}" /></delete>`;
      changes += '</osmChange>';
      continuation();
      return changes;
    }
