...
export default class DeleteAction {
  ...
  public DoDelete(reason: string, onNotAllowed: () => void): UIEventSource<boolean> {
    ...
    this.canBeDeleted.addCallbackAndRun((canBeDeleted) => {
      ...
      OsmObject.DownloadObject(self._id).addCallbackAndRun((obj) => {
        ...
        State.state.osmConnection.changesetHandler.DeleteElement(obj, State.state.layoutToUse.data, reason, State.state.allElements, () => {
          isDeleted.setData(true);
        });
      });
    });
    ...
    const deletetionAllowed = deletingPointsOfOtherAllowed.map(
      (isAllowed) => {
        ...
        if (allByMyself.data === null && useTheInternet) {
          ...
          OsmObject.DownloadHistory(id)
            .map((versions) => versions.map((version) => version.tags['_last_edit:contributor:uid']))
            .syncWith(previousEditors);
        }
