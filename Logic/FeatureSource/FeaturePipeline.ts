...
export default class FeaturePipeline implements FeatureSource {
  ...
  constructor(
    flayers: UIEventSource<{ isDisplayed: UIEventSource<boolean>; layerDef: LayerConfig }[]>,
    ...
    const amendedOverpassSource = new RememberingSource(
      new LocalStorageSaver(
        new MetaTaggingFeatureSource(allLoadedFeatures, new FeatureDuplicatorPerLayer(flayers, new RegisteringFeatureSource(updater))),
        layout
      )
    );

    const geojsonSources: FeatureSource[] = GeoJsonSource.ConstructMultiSource(flayers.data, locationControl).map((geojsonSource) => {
    ...
    const amendedLocalStorageSource = new RememberingSource(
      new RegisteringFeatureSource(new FeatureDuplicatorPerLayer(flayers, new LocalStorageSource(layout)))
    );

    newPoints = new MetaTaggingFeatureSource(
      allLoadedFeatures,
      new FeatureDuplicatorPerLayer(flayers, new RegisteringFeatureSource(newPoints))
    );

    const amendedOsmApiSource = new RememberingSource(
      new MetaTaggingFeatureSource(
        allLoadedFeatures,
        new FeatureDuplicatorPerLayer(
          flayers,
    ...
    this.features = new WayHandlingApplyingFeatureSource(
      flayers,
      new FilteringFeatureSource(flayers, locationControl, selectedElement, merged)
    ).features;
  }
}
