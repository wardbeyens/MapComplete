...
export default class MetaTaggingFeatureSource implements FeatureSource {
   ...
   */
  constructor(
    allFeaturesSource: UIEventSource<{ feature: any; freshness: Date }[]>,
    source: FeatureSource,
    updateTrigger?: UIEventSource<any>
  ) {
    const self = this;
    ...
    function update() {
      ...
      MetaTagging.addMetatags(featuresFreshness, allFeaturesSource, State.state.knownRelations.data, State.state.layoutToUse.data.layers);
