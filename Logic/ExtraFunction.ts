...
export class ExtraFunction {
  static readonly intro = new Combine([
    ...
    new List(['`area` contains the surface area (in square meters) of the object', '`lat` and `lon` contain the latitude and longitude']),
      ...
      return (features) => {
        ...
        for (const otherFeature of features) {
          ...
          if (otherFeature._lon !== undefined && otherFeature._lat !== undefined) {
            ...
            distance = GeoOperations.distanceBetween(GeoOperations.centerpointCoordinates(otherFeature), [feature._lon, feature._lat]);
          }
  ...
  private readonly _f: (
    params: { featuresPerLayer: Map<string, any[]>; relations: { role: string; relation: Relation }[] },
    feat: any
  ) => any;
  ...
  public static HelpText(): BaseUIElement {
    ...
    return new Combine([ExtraFunction.intro, new List(ExtraFunction.allFuncs.map((func) => func._name)), ...elems]);
  }
