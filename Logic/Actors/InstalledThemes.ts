...
export default class InstalledThemes {
  ...
  constructor(osmConnection: OsmConnection) {
    this.installedThemes = osmConnection.preferencesHandler.preferences.map<{ layout: LayoutConfig; definition: string }[]>(
      (allPreferences) => {
  ...
  private static DeleteInvalid(osmConnection: OsmConnection, invalidThemes: any[]) {
    for (const invalid of invalidThemes) {
      ...
      osmConnection.GetLongPreference('installed-theme-' + invalid).setData(null);
    }
  }
}
