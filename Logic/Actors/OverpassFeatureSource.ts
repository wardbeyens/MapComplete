...
export default class OverpassFeatureSource implements FeatureSource {
   ...
   */
  constructor(location: UIEventSource<Loc>, layoutToUse: UIEventSource<LayoutConfig>, leafletMap: UIEventSource<L.Map>) {
    this._location = location;
    this._layoutToUse = layoutToUse;
    this._leafletMap = leafletMap;
    const self = this;

    this.sufficientlyZoomed = location.map(
      (location) => {
  ...
  private IsInBounds(bounds: Bounds): boolean {
    ...
    return b.getSouth() >= bounds.south && b.getNorth() <= bounds.north && b.getEast() <= bounds.east && b.getWest() >= bounds.west;
  }
}
