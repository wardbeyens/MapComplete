...
export class Wikimedia extends ImageAttributionSource {
  ...
  static GetCategoryFiles(
    categoryName: string,
    handleCategory: (ImagesInCategory: ImagesInCategory) => void,
    alreadyLoaded = 0,
  ...
  SourceIcon(backlink: string): BaseUIElement {
    ...
    return new Link(Svg.wikimedia_commons_white_img, `https://commons.wikimedia.org/wiki/${backlink}`, true);
  }
  ...
  PrepareUrl(value: string): string {
    ...
    return Wikimedia.ImageNameToUrl(value, 500, 400).replace(/'/g, '%27');
  }
