  ...
  constructor() {
    super('OsmConnectionSpec-test', [
      [
        'login on dev',
        () => {
          const osmConn = new OsmConnection(false, new UIEventSource<string>(undefined), 'Unit test', true, 'osm-test');
          ...
          osmConn.userDetails.map((userdetails: UserDetails) => {
            if (userdetails.loggedIn) {
              ...
              osmConn.GetPreference('https://raw.githubusercontent.com/AgusQui/MapCompleteRailway/main/railway').setData(data);
            }
          });
