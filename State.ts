...
export default class State {
   ...
   */
  public readonly knownRelations = new UIEventSource<Map<string, { role: string; relation: Relation }[]>>(
    undefined,
   ...
   */
  public readonly LastClickLocation: UIEventSource<{ lat: number; lon: number }> = new UIEventSource<{ lat: number; lon: number }>(
    undefined
  );
  ...
  public layerControlIsOpened: UIEventSource<boolean> = QueryParameters.GetQueryParameter(
  ...
  constructor(layoutToUse: LayoutConfig) {
    ...
    // -- Location control initialization
    {
      const zoom = State.asFloat(
        QueryParameters.GetQueryParameter('z', '' + (layoutToUse?.startZoom ?? 1), 'The initial/current zoom level').syncWith(
          LocalStorageSource.Get('zoom')
        )
      );
      const lat = State.asFloat(
        QueryParameters.GetQueryParameter('lat', '' + (layoutToUse?.startLat ?? 0), 'The initial/current latitude').syncWith(
          LocalStorageSource.Get('lat')
        )
      );
      const lon = State.asFloat(
        QueryParameters.GetQueryParameter('lon', '' + (layoutToUse?.startLon ?? 0), 'The initial/current longitude of the app').syncWith(
          LocalStorageSource.Get('lon')
        )
      );
    ...
    // Feature switch initialization - not as a function as the UIEventSources are readonly
    {
      ...
      this.featureSwitchSearch = featSw('fs-search', (layoutToUse) => layoutToUse?.enableSearch ?? true, 'Disables/Enables the search bar');
      ...
      this.featureSwitchWelcomeMessage = featSw('fs-welcome-message', () => true, 'Disables/enables the help menu or welcome message');
      this.featureSwitchIframe = featSw('fs-iframe', () => false, 'Disables/Enables the iframe-popup');
    ...
    this.osmConnection = new OsmConnection(
      ...
      QueryParameters.GetQueryParameter('oauth_token', undefined, 'Used to complete the login'),
      layoutToUse?.id,
      true,
      // @ts-ignore
    ...
    this.mangroveIdentity = new MangroveIdentity(this.osmConnection.GetLongPreference('identity', 'mangrove'));
