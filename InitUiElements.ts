...
export class InitUiElements {
  static InitAll(
    ...
    if (layoutToUse === undefined) {
      ...
      new FixedUiElement(`Error: incorrect layout <i>${layoutName}</i><br/><a href='https://${window.location.host}/'>Go back</a>`)
    ...
    if ((window != window.top && !State.state.featureSwitchWelcomeMessage.data) || State.state.featureSwitchIframe.data) {
      ...
      const url = `${window.location.origin}${window.location.pathname}?z=${currentLocation.data.zoom ?? 0}&lat=${
        currentLocation.data.lat ?? 0
      }&lon=${currentLocation.data.lon ?? 0}`;
      new MapControlButton(new Link(Svg.pop_out_img, url, true).SetClass('block w-full h-full p-1.5')).AttachTo('messagesbox');
    }
    ...
    const geolocationButton = new Toggle(
      new MapControlButton(new GeoLocationHandler(State.state.currentGPSLocation, State.state.leafletMap, State.state.layoutToUse)),
      undefined,
      State.state.featureSwitchGeolocation
    );

    const plus = new MapControlButton(Svg.plus_ui()).onClick(() => {
      State.state.locationControl.data.zoom++;
      State.state.locationControl.ping();
    });

    const min = new MapControlButton(Svg.min_ui()).onClick(() => {
      State.state.locationControl.data.zoom--;
      State.state.locationControl.ping();
    });

    new Combine([plus, min, geolocationButton].map((el) => el.SetClass('m-0.5 md:m-1'))).SetClass('flex flex-col').AttachTo('bottom-right');
  ...
  private static InitWelcomeMessage() {
    ...
    new Toggle(fullOptions.SetClass('welcomeMessage'), help, isOpened).AttachTo('messagesbox');
  ...
  private static InitLayerSelection(featureSource: FeatureSource) {
    ...
    const copyrightButton = new Toggle(copyrightNotice, new MapControlButton(Svg.osm_copyright_svg()), copyrightNotice.isShown)
      .ToggleOnClick()
      .SetClass('p-0.5');

    const layerControlPanel = new LayerControlPanel(State.state.layerControlIsOpened).SetClass('block p-1 rounded-full');
    ...
    const layerControl = new Toggle(layerControlButton, '', State.state.featureSwitchLayers);
  ...
  private static InitBaseMap() {
    ...
    new LayerResetter(
      ...
      State.state.availableBackgroundLayers,
      State.state.layoutToUse.map((layout: LayoutConfig) => layout.defaultBackgroundId)
    );
    ...
    const bm = new Basemap('leafletDiv', State.state.locationControl, State.state.backgroundLayer, State.state.LastClickLocation, attr);
